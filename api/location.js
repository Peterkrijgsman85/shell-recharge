...
export default async function handler(req, res) {
  ...
  await Promise.all(ids.map(async id => {
    try {
      ...
      console.error(`Fout bij locatie ${id}:`, err);
      // Voeg een placeholder toe met foutmelding
      results.push({
        id,
        address: `Kon locatie ${id} niet laden`,
        evse_1_status: "Onbekend",
        evse_2_status: "Onbekend",
        evse_1_updated: "Onbekend",
        evse_2_updated: "Onbekend"
      });
    }
  }));
